---
description: Project task documentation for the canonical task-workflow plugin.
---

# Task Workflow

`tasks/` is the durable record of human intention, agent planning, execution, and review. The entry point is `/task-workflow:task-start`.

The orchestrator chooses current branch, feature branch, or worktree from repository context. Project instructions may add mockup, sign-off, PR, merge, or deployment gates. They must not weaken the canonical Intent Contract, `DONE_WHEN`, review evidence, or Git-safety rules.
...
8. Choose and record the runtime strategy.
9. Offer `/task-workflow:intent-harden` before sending non-trivial work to the planner.
...
- Record the selected strategy, path/branch, rationale, and delivery expectation in `main.md`.
